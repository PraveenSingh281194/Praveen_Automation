...
import numpy as np
import configpy
def preprocessing(df,month_name,year,loc_list,df_path=None):
    ...
    ignore_cols=['S.No.', 'Unnamed: 3', 'Unnamed: 31','Unnamed: 32','Unnamed: 33','Unnamed: 34','Unnamed: 35','Unnamed: 36','Unnamed: 37','Unnamed: 38','Unnamed: 39','Unnamed: 40']
...
def fiscal_absent_creation(df,date_columns,month_name):
    
    df['Fiscal_Absent_date']=  df[date_columns].applymap(lambda x: 'PL' in x).dot(df[date_columns].columns + ',').str[:-1] + ',' + df[date_columns].applymap(lambda x: 'UL' in x).dot(df[date_columns].columns + ',').str[:-1] + ','  + df[date_columns].applymap(lambda x: 'SL' in x).dot(df[date_columns].columns + ',').str[:-1] + ',' + df[date_columns].applymap(lambda x: 'PH' in x).dot(df[date_columns].columns + ',').str[:-1]
    df['Fiscal_Absent_date']=  df['Fiscal_Absent_date'].str.lstrip(',')
    df['Fiscal_Absent_date']=  df['Fiscal_Absent_date'].str.rstrip(',')
    df['Fiscal_Absent_date'] = df['Fiscal_Absent_date'].str.replace(',,,',',')
    df['Fiscal_Absent_date'] = df['Fiscal_Absent_date'].str.replace(',,',',')
    df['Fiscal_Absent_date'] = df['Fiscal_Absent_date'].str.replace('BD_',f'{month_name}_')
    return df

def cal_absent_creation(df,date_columns,month_name):
    df[date_columns] = df[date_columns].fillna('PL')
    df['Cal_Absent_date']=  df[date_columns].applymap(lambda x: 'PL' in x).dot(df[date_columns].columns + ',').str[:-1] + ',' + df[date_columns].applymap(lambda x: 'UL' in x).dot(df[date_columns].columns + ',').str[:-1] + ','  + df[date_columns].applymap(lambda x: 'SL' in x).dot(df[date_columns].columns + ',').str[:-1] + ',' + df[date_columns].applymap(lambda x: 'PH' in x).dot(df[date_columns].columns + ',').str[:-1]
    df['Cal_Absent_date']=  df['Cal_Absent_date'].str.lstrip(',')
    df['Cal_Absent_date']=  df['Cal_Absent_date'].str.rstrip(',')
    df['Cal_Absent_date'] = df['Cal_Absent_date'].str.replace(',,,',',')
    df['Cal_Absent_date'] = df['Cal_Absent_date'].str.replace(',,',',')
    df['Cal_Absent_date'] = df['Cal_Absent_date'].str.replace('BD_',f'{month_name}_')
    #.replace('_Month2','')
    df['Cal_Absent_date'] = df['Cal_Absent_date'].str.replace('_Month2','')
     
    #var=df['Cal_Absent_date'].values.tolist()
    #df.drop(['Cal_Absent_date'],axis=1,inplace=True)
    #return var
    return df


def date_change():
    #this function takes fiscal dates in 2 lists and converts in Req Date format (example 'BD_1')
    #fiscal_dict dictionary in configpy file to handle fiscal dates
    fiscal_dict=configpy.fiscal_dict
    date_mon_1=fiscal_dict[configpy.month2][1]
    date_mon_2=fiscal_dict[configpy.month2][2]

    list1,list2=[],[]
    list1.extend(['BD_{}_x'.format(i) for i in date_mon_1])
    list2.extend(['BD_{}_y'.format(i) for i in date_mon_2])
    list3=['BD_{}'.format(i) for i in range(1,32)]
    return list1,list2,list3

#func to insert columns at specific values
def fiscal_efforts(df,week,col_name,pos):
    df.insert(pos,col_name,((df[week] == 'Y').sum(axis=1) + (df[week] == 'Working from Mexico').sum(axis=1))*8)
    return df
...
def absent_dates(final_df):
    final_df.insert(len(final_df.columns),'Absent_dates',final_df['Fiscal_Absent_date_x'].map(str) + ',' + final_df['Fiscal_Absent_date_y'].map(str))
    ...
    final_df.drop(['Fiscal_Absent_date_x','Fiscal_Absent_date_y'],axis=1,inplace=True)
    return final_df
...
def calculate(df,date_columns):
    #calculation of present days and leaves of a resource in a month
    df['Fiscal_Working_days'] = (df[date_columns] == 'Y').sum(axis=1) + (df[date_columns] == 'Working from Mexico').sum(axis=1)             
    df['Fiscal_Non_billable_days'] = (df[date_columns].isna()).sum(axis=1)
    df = df.astype(str)
    df[date_columns] = df[date_columns].replace('nan','')
    return df